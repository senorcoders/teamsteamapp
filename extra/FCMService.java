...
public class FCMService extends FirebaseMessagingService implements PushConstants {
  ...
  public void onMessageReceived(RemoteMessage message) {
    ...
    if (extras != null && isAvailableSender(from)) {
      ...
      //send push api
      Log.d(LOG_TAG, "onMessage - from: "+ "init post http");
      try{
        URL url = new URL("https://api.lockerroomapp.com/notification-delivered");
        ...
        for (String key : keys) {
          try {
            ...
            jsonParam.put(key, JSONObject.wrap(extras.get(key)));
          } catch(JSONException e) {
            //Handle exception here
          }
        }
        Log.d("JSON", "onMessage - from: "+ jsonParam.toString());
        ...
        Log.d("STATUS", "onMessage - from: "+ String.valueOf(conn.getResponseCode()));
        Log.d("MSG" , conn.getResponseMessage());

        conn.disconnect();
      }
      catch (Exception e){
        Log.d(LOG_TAG, "onMessage - from: "+ e.getMessage());
      }
    }
  }
