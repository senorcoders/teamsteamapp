...
@IonicPage()
@Component({
  selector: 'page-chat-manager-of-teams-league',
  templateUrl: 'chat-manager-of-teams-league.html',
})
export class ChatManagerOfTeamsLeaguePage {

  public static __name = "ChatManagersPage"
  ...
  public static eventChat: Function = (payload: any) => {
    if (ChatManagerOfTeamsLeaguePage.enableChat === false) return;
    ChatManagerOfTeamsLeaguePage.changeMessages(payload);
  }
  ...
  deleteNotificationLocal() {
    ...
    let index = ListChatsPage.newMessages.findIndex(function (id) { return id === "chatManagers"; });
    if (index !== -1) {
  ...
  private async loadingChatsUp() {
    try {
      let mgs = await this.http.get(`/chatmanagers?where={"league":"${this.league.id}"}&sort=dateTime DESC&limit=20&skip=${this.skip}`).toPromise() as any[];
      mgs = mgs.filter(it => {
  ...
  async ionViewDidEnter() {
    ...
    ChatManagerOfTeamsLeaguePage.enableChat = true;
    ...
    this.subscription.subscribeWithPush("chatManagers", function (msg) {
      // console.log(msg);
      this.pushNewMsg(msg);
    }.bind(this));
  ...
  ionViewWillLeave() {
    // unsubscribe
    ChatManagerOfTeamsLeaguePage.enableChat = false;
    window.clearInterval(this.updateTimeMessage);
    this.events.unsubscribe('chatManagers');
  }
  ...
  */
  private async getMsg() {
    try {
      let mgs = await this.http.get(`/chatmanagers?where={"league":"${this.league.id}"}&sort=dateTime DESC&limit=20`).toPromise() as any[];
      mgs = mgs.filter(it => {
        return it.hasOwnProperty("user");
      });
      this.msgList = mgs.map(function (item) {
  ...
  */
  sendMsg() {
    ...
    this.http.post("/chatmanagers", newMsgPost).toPromise()
      .then((msg: any) => {
  ...
  private async sendImage(data) {
    ...
    try {
      ...
      msg = await this.http.post("/api/chat-managers-teams-league/image", msg).toPromise() as any;
  ...
  public urlImg(id: string) {
    return interceptor.transformUrl(`/api/image/chat_managers_teams_league/${id}`);
  }
